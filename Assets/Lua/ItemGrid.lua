...
ItemGrid.Text = nil
ItemGrid.gridType = nil

--成员函数
--实例化格子对象
function ItemGrid:Init(father,posX,posY,gridType)
    if gridType then
...
function ItemGrid:OnClick()
    if self.data then
        if self.gridType == GridType.shop then
            -- 商店格子
            if ShopPanel and ShopPanel.itemDes then
                ShopPanel.itemDes:ShowItemInfo(self.data)
            end
        else
            -- 背包格子
            if BagPanel and BagPanel.itemDes then
                BagPanel.itemDes:ShowItemInfo(self.data)
            end
        end
    end
end
